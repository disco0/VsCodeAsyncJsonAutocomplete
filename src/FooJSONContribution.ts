...
const localize = (a: string, b: string, c?: string) => c ? c : b;
const LIMIT = 40;
...
export class FooJSONContribution implements IJSONContribution {
    ...
    public collectDefaultSuggestions(_fileName: string, result: ISuggestionsCollector): Thenable<any> {
        ...
        const proposal = new vscode.CompletionItem(localize('json.package.default', 'Default package.json'));
        proposal.kind = vscode.CompletionItemKind.Module;
        proposal.insertText = new vscode.SnippetString(JSON.stringify(defaultValue, null, '\t'));
        result.add(proposal);
    ...
    private onlineEnabled() {
        return !!vscode.workspace.getConfiguration('npm').get('fetchOnlinePackageInfo');
    }
            ...
            if (currentWord.length > 0) {
                if (currentWord[0] === '@') {
                    ...
                    for (let scope of this.knownScopes) {
                        const proposal = new vscode.CompletionItem(scope);
                        proposal.kind = vscode.CompletionItemKind.Property;
                        proposal.insertText = new vscode.SnippetString().appendText(`"${scope}/`).appendTabstop().appendText('"');
                ...
                this.mostDependedOn.forEach((name) => {
                    const insertText = new vscode.SnippetString().appendText(JSON.stringify(name));
                    if (addValue) {
        ...
        if ((location.matches(['dependencies', '*']) || location.matches(['devDependencies', '*']) || location.matches(['optionalDependencies', '*']) || location.matches(['peerDependencies', '*']))) {
            ...
            if (typeof currentKey === 'string') {
                ...
                if (info && info.version) {
                    ...
                    proposal = new vscode.CompletionItem(name);
                    proposal.kind = vscode.CompletionItemKind.Property;
                    proposal.insertText = name;
    ...
    private getDocumentation(description: string | undefined, version: string | undefined, homepage: string | undefined): vscode.MarkdownString {
    ...
    public resolveSuggestion(item: vscode.CompletionItem): Thenable<vscode.CompletionItem | null> | null {
        if (item.kind === vscode.CompletionItemKind.Property && !item.documentation) {
    ...
    public getInfoContribution(_fileName: string, location: Location): Thenable<vscode.MarkedString[] | null> | null {
        if (!this.isEnabled()) {
            return null;
        }
        if ((location.matches(['dependencies', '*']) || location.matches(['devDependencies', '*']) || location.matches(['optionalDependencies', '*']) || location.matches(['peerDependencies', '*']))) {
